        ...
        <el-card>
            <div class="flex fvertical mrb_20">
              <el-button type="primary" @click="getData()" :loading="pageData.loading" >
                    ...
                    <span style="padding-left: 8px;">Show Detail</span>
                </el-button>
...
import { viewMine, deleteRecipe, detailRecipe } from '../../api/common';
@Component({})
export default class recipeDetail extends Vue {
    ...
    readonly tableColumns2 = [
        ...
        { label: "comments", prop: "comments", minWidth: 1000, width: "" },  
    ]
    async getData() {
        ...
        console.log("detail recipe >>", res.data.data.steps);
        var v1=[{}] as never[]
        var v2=[{}] as never[]
        var v3=[{}] as never[]
        var v4=[{}] as never[]
        ...
        for(var i = 0; i < content.comment.length; i++){
                v4[i]={rating:content.comment[i].rating,comments:content.comment[i].comment} as never
        }
        if (res.status === 1) {
